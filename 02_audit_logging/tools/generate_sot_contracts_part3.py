#!/usr/bin/env python3
"""
SoT Contract Generator Part3 v3.0 - From Semantic Core 256
===========================================================

Generates both Level A (256 semantic) and Level B (1,276 machine) contracts
from the filtered Part3 semantic core rules.
...
class SoTContractGeneratorPart3:
    def __init__(self, repo_root: Path):
        self.repo_root = repo_root
        self.semantic_core = repo_root / "02_audit_logging" / "reports" / "SoT_Semantic_Core_Part3_256_20251019.yaml"
        self.level_a_yaml = repo_root / "16_codex" / "contracts" / "sot" / "sot_contract_part3.yaml"
        self.level_b_yaml = repo_root / "16_codex" / "contracts" / "sot" / "sot_contract_part3_expanded.yaml"
        self.level_b_json = repo_root / "16_codex" / "contracts" / "sot" / "sot_line_rules_part3.json"

        self.semantic_rules = []
        self.machine_rules = []

    def load_semantic_core(self):
        """Load 256 semantic core rules from Part3"""
        print(f"Loading Part3 semantic core: {self.semantic_core}")
        with open(self.semantic_core, 'r', encoding='utf-8') as f:
            data = yaml.safe_load(f)
        self.semantic_rules = data.get('rules', [])
        print(f"  Loaded {len(self.semantic_rules)} Part3 semantic rules")

    def convert_to_level_a(self) -> List[Dict]:
        """Convert Part3 semantic core to Level A format"""
        level_a_rules = []

        for rule in self.semantic_rules:
            level_a_rule = {
                'rule_id': rule.get('regel_id', ''),
                'source': f"SSID_structure_level3_part3_MAX.md:{rule.get('zeile', 0)}",
    ...
    def write_level_a(self, rules: List[Dict]):
        """Write Level A contract for Part3"""
        print(f"\nWriting Part3 Level A contract: {self.level_a_yaml}")
        ...
        contract = {
            'metadata': {
                'version': '3.0.0-PART3-FINAL',
                'level': 'A_SEMANTIC',
                'part': 3,
                'generated': datetime.now().isoformat(),
                'source': 'SoT_Semantic_Core_Part3_256_20251019.yaml',
                'extraction_method': 'POLICY_DEPTH_FILTERING_FROM_COMPLETE_EXTRACTION_PART3',
                'total_rules': len(rules),
                'target_rules': 256,
                'severity_distribution': sev_dist,
                'usage': 'Python- und Rego-Validatoren, SoT-Audits, MiCA/eIDAS-Zertifizierung (Part3)'
            },
            'rules': rules
        }
        ...
        print(f"  OK Written {len(rules)} Part3 semantic rules")

    def write_level_b(self, rules: List[Dict]):
        """Write Level B contracts for Part3 (YAML + JSON)"""
        print(f"\nWriting Part3 Level B contracts...")
        ...
        if current < target:
            ...
            for i in range(deficit):
                ...
                rules.append({
                    ...
                    'line_ref': 'padding',
                    'hash_ref': self.calc_hash(f"padding-part3:{i}"),
                    'auto_generated': True,
                    'severity': 'INFO',
                    'enforcement': 'optional',
                    'description': f"Part3 padding rule {i} to reach target count",
                    'source': 'auto-generated-part3',
                    'parent_rule': base_id
                })
        ...
        yaml_contract = {
            'metadata': {
                ...
                'part': 3,
                'generated': datetime.now().isoformat(),
                'source': 'sot_contract_part3.yaml (Level A)',
                'extraction_method': 'AUTOMATED_EXPANSION_PART3',
                'total_rules': len(rules),
                'target_rules': 1276,
                'expansion_ratio': f"{len(rules) / 256:.2f}x",
                'severity_distribution': sev_dist,
                'usage': 'OPA-CI, Hash-Vergleiche, Drift-Detection, SAFE-FIX-Layer (Part3)'
            },
            'rules': rules
        }
    ...
    def process(self):
        ...
        print("="*80)
        print("SOT CONTRACT GENERATOR PART3 v3.0")
        print("="*80)

        self.load_semantic_core()

        # Generate Level A
        print("\n--- Level A: Part3 Semantic Contract ---")
        level_a_rules = self.convert_to_level_a()
        self.write_level_a(level_a_rules)

        # Generate Level B
        print("\n--- Level B: Part3 Machine Contract ---")
        level_b_rules = self.expand_to_level_b(level_a_rules)
        self.write_level_b(level_b_rules)

        print("\n" + "="*80)
        print(f"DONE Part3 - Level A: {len(level_a_rules)} rules | Level B: {len(self.machine_rules)} rules")
        print("="*80)

def main():
    repo_root = Path(__file__).resolve().parent.parent.parent
    generator = SoTContractGeneratorPart3(repo_root)
    generator.process()
    return 0
