...
doppelt gezählten oder fehlerhaft interpretierten Eintrag.
...
# ============================================================================
# ADDITIONAL SEMANTIC PATTERNS - Merged from parse_sot_rules.py
# ============================================================================
# Patterns 151-160: Semantic Rule Detection (for comprehensive mode)

# 151. HASH_START Markers - Logical rule block boundaries
HASH_START_PATTERN = r'^HASH_START::([A-Z0-9_]+)'
HASH_START_ABC_PATTERN = r'^HASH_START::([ABC])_(.+)'

# 152. Path Anchors - Comment-based file references
PATH_ANCHOR_PATTERN = r'^#\s+([\d]{2}_[a-z_]+/[\w/.]+\.(?:yaml|md|py|rego))'

# 153. German MoSCoW Terms (Deutsche Begriffe)
...
class SoTRuleParserV3:
    ...
    def __init__(self, root_dir: Path, output_dir: Optional[Path] = None, mode: str = 'explicit'):
        ...
        # Extraction mode: 'explicit' (RULE-IDs only) or 'comprehensive' (incl. semantic)
        self.mode = mode
    ...
    def build_relation_graph(self, rules: Dict[str, ExtractedRule]) -> Optional[Any]:
        ...
        - REFERENCES: Rule → Rule (See/Reference patterns)
        - DEPENDS_ON: Rule → Rule (Dependency patterns)
        - IMPLEMENTS: Rule → Legal Framework (GDPR, MiCA, etc.)
        - EXTENDS: Rule → Rule (Inheritance patterns)
        - SUPERSEDES: Rule → Rule (Replacement patterns)
        ...
        try:
            ...
            if cycles:
                ...
                for cycle in cycles[:5]:  # Show first 5
                    self.logger.log_warning(f"  Cycle: {' → '.join(cycle)}")
        except:
            pass
    ...
    def _extract_semantic_rules_comprehensive(self, content: str, file_path: Path) -> List[ExtractedRule]:
        ...
        It extracts semantic rules from:
        - HASH_START blocks
        - PATH_ANCHOR comments
        - German MoSCoW terms (MUSS, SOLL, etc.)
        - MUST EXIST statements
        - Score thresholds
        - Regional scope declarations
        - Boolean control flags
        - Purpose/Goal statements
        """
        ...
        for i, line in enumerate(lines, 1):
            # Pattern 151: HASH_START as rule boundary
            if re.match(HASH_START_PATTERN, line):
                match = re.match(HASH_START_PATTERN, line)
                block_name = match.group(1)
                rule = ExtractedRule(
                    rule_id=f"SEM-HASH-{hashlib.sha256(block_name.encode()).hexdigest()[:8]}",
                    text=f"Logical block boundary: {block_name}",
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=MoSCoWPriority.WONT,
                    context="HASH_START block marker",
                    line_number=i,
                    reality_level=RuleReality.SEMANTIC
                )
                rules.append(rule)

            # Pattern 152: PATH_ANCHOR as file reference rule
            if re.match(PATH_ANCHOR_PATTERN, line):
                match = re.match(PATH_ANCHOR_PATTERN, line)
                referenced_path = match.group(1)
                rule = ExtractedRule(
                    rule_id=f"SEM-PATH-{hashlib.sha256(referenced_path.encode()).hexdigest()[:8]}",
                    text=f"File reference anchor: {referenced_path}",
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=MoSCoWPriority.WONT,
                    context="Path anchor comment",
                    line_number=i,
                    reality_level=RuleReality.SEMANTIC
                )
                rules.append(rule)

            # Pattern 153: German MoSCoW terms
            moscow_de_match = re.search(MOSCOW_DE_PATTERN, line)
            if moscow_de_match:
                moscow_term = moscow_de_match.group(1)
                priority = self._map_german_moscow_to_priority(moscow_term)
                rule = ExtractedRule(
                    rule_id=f"SEM-MOSC-{hashlib.sha256(line.encode()).hexdigest()[:8]}",
                    text=line.strip(),
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=priority,
                    context=f"German MoSCoW: {moscow_term}",
                    line_number=i,
                    reality_level=RuleReality.SEMANTIC
                )
                rules.append(rule)
            ...
            if re.search(MUST_EXIST_PATTERN, line):
                rule = ExtractedRule(
                    rule_id=f"SEM-EXIST-{hashlib.sha256(line.encode()).hexdigest()[:8]}",
                    text=line.strip(),
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=MoSCoWPriority.MUST,
            ...
            threshold_match = re.search(SCORE_THRESHOLD_PATTERN, line)
            if threshold_match:
                threshold_value = threshold_match.group(1)
                rule = ExtractedRule(
                    rule_id=f"SEM-SCORE-{hashlib.sha256(line.encode()).hexdigest()[:8]}",
                    text=line.strip(),
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=MoSCoWPriority.SHOULD,
                    context=f"Score threshold: {threshold_value}",
                    line_number=i,
                    reality_level=RuleReality.SEMANTIC
                )
                rules.append(rule)

            # Pattern 156: Regional scope
            scope_match = re.search(REGIONAL_SCOPE_PATTERN, line)
            if scope_match:
                scope = scope_match.group(1)
                rule = ExtractedRule(
                    rule_id=f"SEM-SCOPE-{hashlib.sha256(line.encode()).hexdigest()[:8]}",
                    text=line.strip(),
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=MoSCoWPriority.WONT,
                    context=f"Regional scope: {scope}",
                    line_number=i,
                    reality_level=RuleReality.SEMANTIC
                )
                rules.append(rule)
            ...
            if bool_match:
                control_name = bool_match.group(1)
                control_value = bool_match.group(2)
                rule = ExtractedRule(
                    rule_id=f"SEM-BOOL-{hashlib.sha256(line.encode()).hexdigest()[:8]}",
                    text=line.strip(),
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=MoSCoWPriority.SHOULD if control_value == 'true' else MoSCoWPriority.WONT,
                    context=f"Boolean control: {control_name}={control_value}",
                    line_number=i,
                    reality_level=RuleReality.SEMANTIC
                )
                rules.append(rule)

            # Pattern 161: Purpose/Goal statements
            purpose_match = re.match(PURPOSE_PATTERN, line)
            if purpose_match:
                purpose_type = purpose_match.group(1)
                purpose_text = purpose_match.group(2)
                rule = ExtractedRule(
                    rule_id=f"SEM-PURP-{hashlib.sha256(line.encode()).hexdigest()[:8]}",
                    text=purpose_text.strip(),
                    source_path=str(file_path),
                    source_type=RuleSource.SEMANTIC_PATTERN,
                    priority=MoSCoWPriority.WONT,
                    context=f"{purpose_type} statement",
                    line_number=i,
                    reality_level=RuleReality.SEMANTIC
                )
                rules.append(rule)

        self.logger.log_info(f"  Comprehensive mode: Extracted {len(rules)} semantic rules from {file_path.name}")
        return rules

    def _map_german_moscow_to_priority(self, moscow_term: str) -> 'MoSCoWPriority':
        """Map German MoSCoW terms to priority levels"""
        mapping = {
            ...
            'OPTIONAL': MoSCoWPriority.WONT,
        }
        return mapping.get(moscow_term, MoSCoWPriority.UNKNOWN)
...
def main():
    ...
    parser_cli = argparse.ArgumentParser(
        description='SoT Rule Parser V4.0 - Unified Extraction Engine',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
MODES:
  explicit        Extract only explicit rules with RULE-IDs (default)
                  Expected: ~4,723 rules from Contract, Policy, Validator, Tests

  comprehensive   Extract explicit + semantic rules from descriptions
                  Expected: ~9,000+ rules including MUST/SHOULD/MAY patterns,
                  HASH_START blocks, path anchors, score thresholds, etc.

EXAMPLES:
  # Explicit mode (backward compatible)
  python sot_rule_parser_v3.py --mode explicit

  # Comprehensive mode (all semantic patterns)
  python sot_rule_parser_v3.py --mode comprehensive

  # With JSON output
  python sot_rule_parser_v3.py --mode comprehensive --output rules.json
        """
    )
    ...
    parser_cli.add_argument(
        ...
        type=str,
        default=None,
        help='Output JSON file path (default: auto-generated in 02_audit_logging/reports/)'
    )

    parser_cli.add_argument(
        '--verbose',
        action='store_true',
        help='Enable verbose output with detailed pattern matching'
    )

    args = parser_cli.parse_args()

    print("=" * 70)
    print(f"SoT Rule Parser V4.0 - Mode: {args.mode.upper()}")
    print("=" * 70)
    print()

    if args.mode == 'comprehensive':
        print("[INFO] Comprehensive mode enabled - extracting semantic rules")
        print("[INFO] This includes MUST/SHOULD/MAY patterns, HASH_START blocks,")
        print("[INFO] path anchors, score thresholds, and 150+ semantic patterns")
        print()
    ...
    print()
    print("Features:")
    print("  ✓ 30 Forensic Layers")
    print("  ✓ 150+ Semantic Patterns")
    print("  ✓ 10 Extended Extraction Methods")
    print("  ✓ NetworkX Relation Graph")
    print("  ✓ 5 Master File Specialization")
    print("  ✓ HASH_START Segment Detection")
    print()
