...
class CoverageChecker:
    ...
    def run(self) -> CoverageReport:
        ...
        for i, rule in enumerate(self.rules, 1):
            # Sanitize rule text for Windows console (replace Unicode with ASCII)
            safe_text = rule.rule_text[:60].replace('→', '->').replace('ü', 'ue').replace('ä', 'ae').replace('ö', 'oe').replace('ß', 'ss').encode('ascii', 'replace').decode('ascii')
            print(f"[{i}/{len(self.rules)}] {rule.rule_id}: {safe_text}...")
...
class ReportGenerator:
    ...
    def generate_console_report(report: CoverageReport):
        ...
        print()
        print(f"[OK]   Full Coverage:    {report.full_coverage_count:3d} ({report.full_coverage_count/report.total_rules*100:.1f}%)")
        print(f"[WARN] Partial Coverage: {report.partial_coverage_count:3d} ({report.partial_coverage_count/report.total_rules*100:.1f}%)")
        print(f"[MISS] No Coverage:      {report.no_coverage_count:3d} ({report.no_coverage_count/report.total_rules*100:.1f}%)")
        print()
        ...
        for result in report.results:
            if not result.full_coverage:
                ...
                print(f"   Contract:  {'[OK]' if result.contract_coverage else '[MISS]'}")
                print(f"   Core:      {'[OK]' if result.core_coverage else '[MISS]'}")
                print(f"   Policy:    {'[OK]' if result.policy_coverage else '[MISS]'}")
                print(f"   CLI:       {'[OK]' if result.cli_coverage else '[MISS]'}")
                print(f"   Test:      {'[OK]' if result.test_coverage else '[MISS]'}")
